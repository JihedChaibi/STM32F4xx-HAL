...
void SPI_Control(SPI_RegDef_t* pSPI, uint8_t ENA_DIS){
 ...
 * @Note              -  none
 */
void SPI_CLKControl(SPI_RegDef_t* pSPIx, uint8_t ENA_DIS){
...
void SPI_Init(SPI_Handle_t *pSPIHandler){
    ...
    // 5. Software slave management config
    temp_reg |= pSPIHandler->SPIConfig.SPI_SSM << SPI_CR1_SSM;


    // 6. Internal slave select
    temp_reg |= pSPIHandler->SPIConfig.SPI_SSI << SPI_CR1_SSI;
 ...
 * @fn      		  - SPI_DeInit
 *
 * @brief             - This function de-initialize the SPI
 *                      to its reset values
 *
 * @param[in]         - base address of the SPI peripheral handler
 * @param[in]         -
 *
 * @return            -  none
 *
...
void SPI_DeInit(SPI_RegDef_t *pSPI){
	...
	if 		(pSPI == SPI1) SPI1_REG_RESET();
	else if (pSPI == SPI2) SPI2_REG_RESET();
	else if (pSPI == SPI3) SPI3_REG_RESET();
	else if (pSPI == SPI4) SPI4_REG_RESET();
 ...
 *
 *
 * @return            -  Flag status : FLAG_RESET / FLAG_SET
 *
 * @Note              -  none
 */
...
void SPI_SendData(SPI_RegDef_t *pSPIx,uint8_t *pTxBuffer, uint32_t Len){
	...
	while(Len > 0)
	{
	  ...
	  // if TX Buffer not empty.. wait
      while(SPI_GetFlagStatus(pSPIx,SPI_TXE_FLAG) == FLAG_RESET );

      if( (pSPIx->CR1 & (1 << SPI_CR1_DFF)) == SPI_DFF_16BITS)
      {
    	  //16 bits data
 ...
 * @param[in]         - base address of the SPI peripheral handler
 * @param[in]         - pointer to data buffer
 * @param[in]         - length of data to read
 *
 *
 * @return            -  none
 *
